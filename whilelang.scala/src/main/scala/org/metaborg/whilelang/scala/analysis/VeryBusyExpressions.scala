...
/**
  * Classical backward+must analysis that collects expressions that will need to be evaluated in all branches after
  * the statement before their free variables are assigned to again.
  */
object VeryBusyExpressions extends ClassicalAnalysis {
  ...
  override def gen(node: RelevantNode): Property = node match {
    case LabeledAssign3(id1, expr2, int3) => Utils.collectAExprs(expr2)
    case LabeledSkip1(int1) => Set.empty
    case Seq2(labeledstatement1, labeledstatement2) => Set.empty
    case LabeledIfThenElse4(expr1, int2, labeledstatement3, labeledstatement4) => Utils.collectAExprs(expr1)
